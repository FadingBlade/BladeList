# BladeList Windows Installer (PowerShell)
# Run with: irm https://raw.githubusercontent.com/FadingBlade/BladeList/main/install.ps1 | iex
...
Write-Host ""
Write-Host "  BladeList installer" -ForegroundColor Cyan -NoNewline
Write-Host " - terminal to-do manager" -ForegroundColor DarkGray
Write-Host ""
...
if (-not $PYTHON) {
    ...
    if ($ans -eq "" -or $ans -match "^[Yy]") {
        ...
        if (-not $PYTHON) {
            ...
            Write-Host ""
            Write-Host "  irm https://raw.githubusercontent.com/FadingBlade/BladeList/main/install.ps1 | iex" -ForegroundColor Cyan
            Write-Host ""
            exit 0
        }
    } else {
...
$INSTALL_DIR = "$env:USERPROFILE\.BladeList"
New-Item -ItemType Directory -Force -Path $INSTALL_DIR | Out-Null
Write-Step "Installing to: $INSTALL_DIR"

# ── Download BladeList.py ─────────────────────────────────────────────────────

$DEST_PY = "$INSTALL_DIR\BladeList.py"
Write-Step "Downloading BladeList..."
Invoke-WebRequest -Uri "$REPO/BladeList.py" -OutFile $DEST_PY -UseBasicParsing
Write-Ok "Downloaded BladeList.py"

# ── Create todo.cmd wrapper ───────────────────────────────────────────────────

$WRAPPER = "$INSTALL_DIR\todo.cmd"
Set-Content -Path $WRAPPER -Value "@echo off`r`n`"$PYTHON`" `"$DEST_PY`" %*"
Write-Ok "Created todo.cmd"
...
Write-Host ""
Write-Ok "BladeList installed!"
Write-Host ""
Write-Host "  Open a NEW terminal window, then type:" -ForegroundColor Yellow
Write-Host ""
Write-Host "    todo add 'buy milk' --high     add a task" -ForegroundColor Cyan
Write-Host "    todo list                      see all tasks" -ForegroundColor Cyan
Write-Host "    todo done <id>                 mark as done" -ForegroundColor Cyan
Write-Host "    todo stats                     see summary" -ForegroundColor Cyan
Write-Host "    todo update                    update BladeList" -ForegroundColor Cyan
Write-Host "    todo help                      show all commands" -ForegroundColor Cyan
Write-Host ""
